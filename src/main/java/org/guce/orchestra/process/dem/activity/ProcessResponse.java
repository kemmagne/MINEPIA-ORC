...
public class ProcessResponse extends CustomActivity {
    ...
    public Object execute(Request request, OrchestraEbxmlMessage ebxmlMessage) throws Exception {
        ...
        if (ebxmlMessage.getAction().equals(DEMConstants.DEM03)) {
            toPartyIds = new String[]{request.getImportateurPartner()};
            request.getDossier().setStatus(Dossier.FILE_STATUS_REJECTED);
        } else if (ebxmlMessage.getAction().equals(DEMConstants.DEM02)) {
            toPartyIds = new String[]{request.getImportateurPartner()};
            request.getDossier().setStatus(Dossier.FILE_STATUS_COMPLEMENT);
        } else if (ebxmlMessage.getAction().equals(DEMConstants.DEM04)) {
            toPartyIds = new String[]{request.getImportateurPartner()};
            request.getDossier().setStatus(Dossier.FILE_STATUS_VALIDATED);
        } else if (ebxmlMessage.getAction().equals(DEMConstants.DEM10)) {
            toPartyIds = new String[]{request.getImportateurPartner()};
            request.getDossier().setStatus(Dossier.FILE_STATUS_VALIDATED);
        } else if (ebxmlMessage.getAction().equals(DEMConstants.DEM601)) {
            toPartyIds = new String[]{request.getRecieverPartner(), request.getImportateurPartner()};
        } else if (ebxmlMessage.getAction().equals(DEMConstants.DEM602)) {
            toPartyIds = new String[]{request.getRecieverPartner(), request.getImportateurPartner()};
        }
        response = (java.util.Map<String, Object>) this.generateResponse(ebxmlMessage);
        if (ebxmlMessage.getAction().equals(DEMConstants.DEM601)) {
            Set<OrchestraEbxmlFields> fields = new HashSet<>();
            fields.add(OrchestraEbxmlFields.ConversationId);
            fields.add(OrchestraEbxmlFields.MessageId);
            fields.add(OrchestraEbxmlFields.TimeStamp);
            ...
            notificationMessage.getMessageHeader().setService("dem");
            String action = "PAY602".equals(ebxmlMessage.getAction()) ? DEMConstants.DEM602 : ebxmlMessage.getAction();
            ...
            OrchestraEbxmlMessage dem01Message = createDEM01Message(request, ebxmlInitMessage);
            request.update();
            response.put("action", DEMConstants.DEM01);
            response.put("vtMessage", dem01Message);
    ...
    public OrchestraEbxmlMessage createDEM01Message(Request request, OrchestraEbxmlMessage ebxmlInitMessage) throws Exception {
        Dossier dossier = request.getDossier();
        Set<OrchestraEbxmlFields> fields = new HashSet<>();
        fields.add(OrchestraEbxmlFields.ConversationId);
        fields.add(OrchestraEbxmlFields.MessageId);
        fields.add(OrchestraEbxmlFields.TimeStamp);
        OrchestraEbxmlMessage dem01Message = OrchestraEbxmlUtility.copyWithout(ebxmlInitMessage, fields);
        dem01Message.getMessageHeader().setRefToMessageId(ebxmlInitMessage.getMessageId());
        dem01Message.setDossier(dossier); 
