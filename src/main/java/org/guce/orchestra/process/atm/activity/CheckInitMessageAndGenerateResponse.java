...
public class CheckInitMessageAndGenerateResponse extends CustomActivity {
    ...
    public Object execute(Request request, OrchestraEbxmlMessage ebxmlMessage) throws Exception {
        ...
        if (ATMConstants.ATM01.equals(ebxmlMessage.getAction())) {
            ...
            Logger.getLogger(this.getClass().getName()).info("START CREATION MESSAGE PAIEMENT ATM");
            OrchestraEbxmlMessage atmPayment = createPaymentMessage(request, ebxmlMessage);
            ((Map<String, Object>) response).put("atmPayment", atmPayment);
            list.add((OrchestraEbxmlMessage) createNotificationMessage(request, ebxmlMessage, ATMConstants.ATM601));
            ((Map<String, Object>) response).put("notification601", list);
            Logger.getLogger(this.getClass().getName()).info("END CREATION MESSAGE PAIEMENT ATM");
        }
        request.update();
        return response;
    }
    ...
    public OrchestraEbxmlMessage createPaymentMessage(Request request, OrchestraEbxmlMessage ebxmlMessage) throws Exception {
        ...
        InputStream xslIs = getClass().getResourceAsStream(ATMConstants.XSL_RESSOURCE_PATH + "payment.xslt");
        ...
        if (ebxmlMessage.getDossier() != null && StringUtils.isNotEmpty(ebxmlMessage.getDossier().getProperty(ATMConstants.ATM_BILL_TYPE))) {
            t.setParameter("param_type_facture", ebxmlMessage.getDossier().getProperty(ATMConstants.ATM_BILL_TYPE));
        }else{
            t.setParameter("param_type_facture", "ATM_"+request.getRecieverPartner());
        }
    ...
    public OrchestraEbxmlMessage createNotificationMessage(Request request, OrchestraEbxmlMessage ebxmlMessage, String notificationCode) throws Exception {
        ...
        Document document = db.parse(new ByteArrayInputStream(data));
        TransformerFactory transformFactory = TransformerFactory.newInstance();
        InputStream xslIs = getClass().getResourceAsStream(ATMConstants.XSL_RESSOURCE_PATH + "atm601.xslt");
        Transformer t = transformFactory.newTransformer(new StreamSource(xslIs));
        ...
        message.getMessageHeader().setService("atm");
